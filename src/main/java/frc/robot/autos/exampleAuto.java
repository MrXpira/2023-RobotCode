...
public class exampleAuto extends SequentialCommandGroup {
    public exampleAuto(Swerve s_Swerve,ArmSubsystem armSubsystem, Vision limelightSubsystem, WinchSubsystem winchSubsystem, ClawSubsystem clawSubsystem){

// This will load the file "FullAuto.path" and generate it with a max velocity of 4 m/s and a max acceleration of 3 m/s^2
// for every path in the group
ArrayList<PathPlannerTrajectory> pathGroup = (ArrayList<PathPlannerTrajectory>) PathPlanner.loadPathGroup("Test Path Copy", new PathConstraints(Constants.AutoConstants.kMaxSpeedMetersPerSecond, Constants.AutoConstants.kMaxAccelerationMetersPerSecondSquared));

// This is just an example event map. It would be better to have a constant, global event map
// in your code that will be used by all path following commands.
HashMap<String, Command> eventMap = new HashMap<>();
//eventMap.put("balanceRobot", s_Swerve.);
// eventMap.put("Movearmandscore", winchSubsystem.moveWinchToPosition(-100).andThen(armSubsystem.moveArmToPosition(25000).andThen(winchSubsystem.moveWinchToPosition(1000).andThen(clawSubsystem.moveClawToPosition(null)))));
