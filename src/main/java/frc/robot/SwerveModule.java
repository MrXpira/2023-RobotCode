...
public class SwerveModule {
    ...
    private void setAngle(SwerveModuleState desiredState){
        Rotation2d angle;
        if (Math.abs(getAngle().getDegrees() - desiredState.angle.getDegrees()) >= .1) {
            angle = desiredState.angle;
        } else {
            angle = (Math.abs(desiredState.speedMetersPerSecond) <= (Constants.Swerve.maxSpeed * 0.01)) ? lastAngle : desiredState.angle; //Prevent rotating module if speed is less then 1%. Prevents Jittering.
        }
